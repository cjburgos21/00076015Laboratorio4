void createLocalA(Matrix &A,mesh m){
    float T = m.getParameter(VALOR_T);
    A.at(0).at(0) += -T/8;  A.at(0).at(1) += T/8;
    A.at(1).at(0) += -T/8;  A.at(1).at(1) += T/8;
}

void createLocalC(Matrix &C,mesh m){
    float PHI = m.getParameter(VALOR_PHI);
    C.at(0).at(0) += -3.0*PHI/2;  C.at(0).at(1) += 3.0*PHI/2;
    C.at(1).at(0) += -3.0*PHI/2;  C.at(1).at(1) += 3.0*PHI/2;
}

void createLocalG(Matrix &G,mesh m){
    float MEW = m.getParameter(VALOR_MEW);
    G.at(0).at(0) += -MEW/3;  G.at(0).at(1) += MEW/3;
    G.at(1).at(0) += -MEW/3;  G.at(1).at(1) += MEW/3;
}

void createLocalI(Matrix &I,mesh m){
    float A = m.getParameter(VALOR_A);
    I.at(0).at(0) += -A/2.0;  I.at(0).at(1) += A/2.0;
    I.at(1).at(0) += -A/2.0;  I.at(1).at(1) += A/2.0;
}

void createLocalP(Matrix &P,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float N = m.getParameter(VALOR_N);
    P.at(0).at(0) += N/l;      P.at(0).at(1) += -N/l;
    P.at(1).at(0) += -N/l;     P.at(1).at(1) += N/l;
}

void createLocalQ(Matrix &Q,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float U = m.getParameter(VALOR_U);
...
Vector createLocalb(int element,mesh &m){
    ...
    float J = m.getParameter(VALOR_J), H = m.getParameter(VALOR_H), l = m.getParameter(ELEMENT_LENGTH);
